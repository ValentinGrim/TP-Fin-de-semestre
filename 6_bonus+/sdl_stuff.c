...
int sdl_startup()
{
    ...
    bmp2 = SDL_LoadBMP("ah.bmp");
    if (!bmp2)
    {
        printf("Unable to load bitmap: %s\n", SDL_GetError());
        return 0;
    }
    SDL_SetColorKey(bmp2, SDL_SRCCOLORKEY, SDL_MapRGB(bmp->format, 0, 0, 0));
    SDL_SetColorKey(bmp2, SDL_SRCCOLORKEY, SDL_MapRGB(bmp2->format, 0, 0, 0));
...
int sdl_loop()
{
    ...
    // dessine les  balles
        ...
        SDL_Rect dstrect;
        dstrect.x =  WINDOW_SIZE*positions[0].x - bmp->w/2;
        dstrect.y =  WINDOW_SIZE - WINDOW_SIZE*positions[0].y - bmp->h/2;
        ...
        dstrect.x =  WINDOW_SIZE*positions[1].x - bmp2->w/2;
        dstrect.y =  WINDOW_SIZE - WINDOW_SIZE*positions[1].y - bmp2->h/2;

	
        // draw bitmap
        SDL_BlitSurface(bmp2, 0, screen, &dstrect);
